...
class Solution:
    ...
    def longestIncreasingPath(self, matrix: List[List[int]]) -> int:
        '''Given an m x n matrix, returns an integer'''
        if(matrix == None):
            return None # Null case
        else:
            self.matrix = matrix
            self.x_length = len(matrix)
            self.y_length = len(matrix[0])
            self.x_range = range(self.x_length)
            self.y_range = range(self.y_length)
            cache = [[-1 for j in range(self.y_length)] for i in range(self.x_length)]
            max_length = 1
            for x in range(self.x_length): # Iterate through all possible coordinates of matrix
    ...
    def search(self, x, y, cache):
        '''Returns a matrix which, at coordinates (x,y) has the length of the largest strictly-increasing path starting from (x,y). Works recursively.'''
        max_length = 1
        if(cache[x][y] != -1):
            return cache
        else:
            # Check adjacent boxes (up, down, left, right) using self.directional array
            for d in self.directional:
                a = x+d[0] # (a, b) = coordinates of the adjacent box in this loop
                b = y+d[1]
                if(a in self.x_range and b in self.y_range and self.matrix[a][b] > self.matrix[x][y]): # if(adjacent block exists AND adjacent > current)
                    if(cache[a][b] == -1): # if(adjacent block NOT cached)
                        cache = self.search(a, b, cache)
                    if(max_length < cache[a][b] + 1): # if(value of adjacent value > max value)
                        max_length = cache[a][b] + 1
